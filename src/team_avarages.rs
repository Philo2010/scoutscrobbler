...
pub async fn get_team_avarage_event(pool: &sqlx::PgPool, event: &str) -> Result<Vec<EntryAvarage>, sqlx::Error> {
    let teams: Vec<i32> = sqlx::query_scalar!(
        r#"
        SELECT DISTINCT team
        FROM scouting_entry
...
#[get("/team_event/<event>")]
pub async fn avarage_team_event(pool: &State<sqlx::PgPool>, event: &str) -> Template {
    let mut avarages = match get_team_avarage_event(pool.inner(), event).await {
        Ok(a) => a,
        Err(_) => {
            return Template::render("error", context![error: "Database Error"]);
        },
